# Olá, eu sou Thales Andrade Biondi 👋

Sou desenvolvedor apaixonado por **tecnologia, lógica de programação e desenvolvimento de projetos criativos**. Atualmente, estou explorando diversas áreas da programação, incluindo **C, C#, HTML, CSS e PHP**, além de projetos de banco de dados e lógica estruturada.

---

## Sobre mim

- 💻 Desenvolvedor com foco em **projetos de lógica, programação estruturada e orientação a objetos**.
- 🛠️ Experiência com **desenvolvimento de scripts, jogos, aplicações web e sistemas locais**.
- 🌱 Sempre em aprendizado contínuo, explorando novas linguagens e frameworks.
- 🎯 Interesse em projetos que unem **criatividade, lógica e soluções práticas**.

---

## Habilidades

- **Linguagens de programação:** C, C#, PHP, HTML, CSS.
- **Conceitos:** POO, algoritmos, estruturas de repetição, estruturas condicionais, manipulação de dados.
- **Ferramentas:** VSCode, IntelliJ, Git/GitHub.
- **Outras competências:** Criação de jogos simples, projetos educacionais, scripts automatizados e prototipagem rápida.

---

## Contato

- 📧 Email: [biodinho20@gmail.com](mailto:biodinho20@gmail.com)  
- 📱 WhatsApp: +55 12 98813-9975  
- 🔗 LinkedIn: [linkedin.com/in/thales-andrade-biondi-84354335b](https://www.linkedin.com/in/thales-andrade-biondi-84354335b/)

---

> 💬 Estou sempre aberto a colaborações, novos projetos e desafios que me permitam crescer como desenvolvedor e resolver problemas de forma criativa.

## Most Used Languages
![Top Langs](https://github-readme-stats-git-masterrstaa-rickstaa.vercel.app/api/top-langs/?username=Thales-biondi12&layout=compact)
